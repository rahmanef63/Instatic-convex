...
Instatic's data layer is a **self-hosted Convex backend**, not a managed database on Railway. The CMS runs as two halves deployed together: the Bun app (repo-root `Dockerfile`) and a self-hosted Convex backend (`convex/compose.selfhosted.yml`) whose entire database lives in the persistent `instatic_convex_data` volume. There is no `DATABASE_URL`, no managed Postgres, and no SQLite disk to provision here.
