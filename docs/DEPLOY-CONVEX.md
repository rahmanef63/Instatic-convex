...
Moving the Instatic CMS data layer onto a **self-hosted Convex** backend and
serving the app from **Dokploy** at `instatic.rahmanef.com`.

> **STATUS: LIVE.** The SQL→Convex rewrite is complete — the Bun server reads/writes
> exclusively through Convex (`server/convex/client.ts`); `server/db/*` is gone. The
> self-hosted Convex backend is deployed (steps A+B done, schema pushed) and the app
> is served from Dokploy. All three steps below are runnable; none are blocked.
...
The data-layer rewrite that once gated step C is done: every `server/` repository
delegates to Convex through `server/convex/client.ts`, `server/db/*` is deleted,
`convex/_generated` is committed, and `bun run build` / `bun test` are green. All
three steps above run unconditionally.
